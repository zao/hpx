...
namespace hpx { namespace performance_counters { namespace memory
{
    ...
    std::uint64_t read_psm_virtual(bool)
    {
        ...
        struct kinfo_proc2* pp = kvm_getproc2(kvm.get(), KERN_PROC_PID, getpid(), sizeof(kinfo_proc2), &proc_count);
#endif
        if (!pp)
        {
            HPX_THROW_EXCEPTION(kernel_error,
...
#elif defined(__NetBSD__)
        std::uint64_t virtual_size = static_cast<std::uint64_t>(pp->p_vm_msize) * NBPG;
#endif
...
#elif defined(__NetBSD__)
        std::uint64_t resident_size = static_cast<std::uint64_t>(pp->p_vm_rssize) * NBPG;
#endif
